...
#define TIME 10000
    ...
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_ADDR)<<8) |  MPU_AK8963_ADDRESS|  0x80); //  Set the I2C slave address of AK8963 and set for read.
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(TIME);
    // ****************** END     CONFIGURAMOS I2C dirección base  ************ //
    // ****************** BEGIN     Escribimos registo que deseamos leer  ************ //
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_REG)<<8) |  regRead); // I2C slave 0 register address from where to begin data transfer
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(TIME);
    // ****************** END     Escribimos registo que deseamos leer  ************ //
    // ****************** BEGIN     indicamos datos que van a leerse  ************ //
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_CTRL)<<8) |  0x81);   // 8: habilitamos lectura del esclavo para almacenar en SLV0. 1: un byte. I2C slave 0 register address from where to begin data transfer, Enable I2C and transfer 1 byte
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(TIME);
    // ****************** END       indicamos datos que van a leerse  ************ //
    // ****************** BEGIN     Leemos registro donde se escribió el dato  ************ //
    ComandoSPI[0]=(((MPU_READ | MPU_EXT_SENS_DATA_00)<<8));      // Read the WHO_AM_I byte
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(TIME);
    ReadOut[0] = ( DatoSPI01[0]);
...
void write_AK8963(spiBASE_t *spiREGISTRO, spiDAT1_t *SPIconfig, int16_t regwrite, int16_t *ReadOut, int16_t dataWrite)
{
    ...
    // ****************** BEGIN     CONFIGURAMOS I2C dirección base  ************ //
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_ADDR)<<8) |  MPU_AK8963_ADDRESS|  0x00); //  Set the I2C slave address of AK8963 and set for read.
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    // ****************** BEGIN     CONFIGURAMOS I2C dirección base  ************ //
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_REG)<<8) |  regwrite); //  Set the I2C slave address of AK8963 and set for read.
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    // ****************** END     CONFIGURAMOS I2C dirección base  ************ //
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_DO)<<8) |  dataWrite); //  Set the I2C slave address of AK8963 and set for read.
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    // ****************** END     CONFIGURAMOS I2C dirección base  ************ //
...
boolean mpuGetGyroAccel(spiBASE_t *spiREGISTRO, spiDAT1_t *SPIconfig, int *Gyro, int *Accel)
{
    ...
    ComandoSPI[0]=(MPU_GYRO_XOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_GYRO_XOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
    Gyro[X]=(int32_t)((int16_t)(SPI_Dat_respH_L));

    /*-----------------| Gy |--------------------*/

    ComandoSPI[0]=(MPU_GYRO_YOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_GYRO_YOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
    ...
    ComandoSPI[0]=(MPU_GYRO_ZOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_GYRO_ZOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
    ...
    ComandoSPI[0]=(MPU_ACCEL_XOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_ACCEL_XOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
    ...
    ComandoSPI[0]=(MPU_ACCEL_YOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_ACCEL_YOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
    ...
    ComandoSPI[0]=(MPU_ACCEL_ZOUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = ( DatoSPI01[0]<<8);
    ComandoSPI[0]=(MPU_ACCEL_ZOUT_L | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
...
boolean read_MPU_ACEL_TMP_VF(spiBASE_t *spiREGISTRO, spiDAT1_t *SPIconfig, int *ReadOut_TMP_Giro_Acc, boolean imprimir_SN_TMP, boolean imprimir_SN_GIRO, boolean imprimir_SN_ACEL)
{
    ...
    ComandoSPI[0]=(MPU_TEMP_OUT_H | MPU_READ)<<8; // Preguntamos quien es
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    ComandoSPI[0]=(MPU_TEMP_OUT_L | MPU_READ)<<8; // Preguntamos quien es
    wait(5000);
    DatoSPI01[0]=(DatoSPI01[0] & 0x00FF);
    SPI_Dat_respH_L = (SPI_Dat_respH_L | DatoSPI01[0]);
...
{
    wait(5000);
    ComandoSPI[0]=(((MPU_WRITE | MPU_GYRO_CONFIG)<<8)  |  0x18); // configuramos sensibilidad del giroscopio
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
}
void config_AK8963(spiBASE_t *spiREGISTRO, spiDAT1_t *SPIconfig)
{
    ...
    ComandoSPI[0]=(((MPU_WRITE | MPU_USER_CTRL)<<8)  |  0x20); // // Enable I2C Master mode
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(50000);
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_MST_CTRL)<<8)  |  0x0D); // I2C configuration multi-master I2C 400KHz
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(50000);
...
{
    wait(5000);
    ComandoSPI[0]=(((MPU_WRITE | MPU_GYRO_CONFIG)<<8)  |  0x18);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
}
...
void config_AK8963_VF(spiBASE_t *spiREGISTRO, spiDAT1_t *SPIconfig)
{
    ...
    wait(5000);
    ComandoSPI[0]=(((MPU_WRITE | MPU_USER_CTRL)<<8)  |  0x20);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_MST_CTRL)<<8)  |  0x0D);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(5000);
...
{
    int16_t readout2[1]= {0};
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_ADDR)<<8) |  MPU_AK8963_ADDRESS|  0x00);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(500);
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_REG)<<8) |  regwrite);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(500);
    ComandoSPI[0]=(((MPU_WRITE | MPU_I2C_SLV0_DO)<<8) |  dataWrite);
    spiSendAndGetData(spiREGISTRO, SPIconfig,(uint32) 1, ComandoSPI,DatoSPI01);
    wait(1000);
...
//  sciEnviarDatos(sprintf(command, " "),command, 0);
    //unsigned char ReceivedDataTMP01[1];
    //unsigned int i_sci=0,i_sciAux=0;
    //unsigned char SCI_ReceivedData[4];

//    sciReceive(scilinREG,1,SCI_ReceivedData);
//    sciEnviarDatos(sprintf(command, "%c", SCI_ReceivedData[0]),command, 0);


    /*
            sciEnviarDatos(sprintf(command," :x: %7.7f",CoordGeneralUTM.x),command, 0);
            sciEnviarDatos(sprintf(command," :y: %7.7f",CoordGeneralUTM.y),command, 0);
            sciEnviarDatos(sprintf(command," :z: %7.7f",CoordGeneralUTM.z),command, 1);
    */
